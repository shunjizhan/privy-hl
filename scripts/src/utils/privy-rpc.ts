...
/**
 * Call Privy API directly (for Admin) or via Gateway (for Operator)
 */
export const callPrivyRpc = async (params: CallPrivyRpcParams): Promise<Hex> => {
  ...
  // Admin calls Privy directly, Operator goes through Gateway
  if (privyAppSecret) {
  ...
  // Operator path - via Gateway
  const response = await axios.post(
    `${gatewayUrl}/v1/vault/${walletId}/rpc`,
    body,
    {
