...
vaultRoutes.get('/health', (c) => {
  return c.json({ status: 'ok', service: 'smart-vault-be' });
});
...
vaultRoutes.post('/create', async (c) => {
  try {
    ...
    const withdrawWhitelist = parsed.data.withdrawWhitelist || [config.WHITELISTED_ADDRESS];
...
vaultRoutes.post('/:walletId/rpc', async (c) => {
  ...
  console.log('[RPC] Incoming request:');
  console.log('  Wallet ID:', walletId);
  console.log('  Payload:', JSON.stringify(bodyResult.data, null, 2));
